...
function updateGeneralProgramPage() {
	...
	const programName = $("#program-name").val();
	const minCost = parseInt($("#min-cost").val());
	const maxCost = parseInt($("#max-cost").val());
	...
	if (removeInitFile) {
		$.post("../../../utility/functions.php", {
			functionCall: "deleteGeneralMentoringProposalFile",
			pageID: pageID,
	...
	if (isEmpty(programName) || isEmpty(minCost) || isEmpty(maxCost) || isEmpty(programDuration) || isEmpty(focusProblemDesc) || isEmpty(about) || isEmpty(expectedOutcome)) {
	...
	if (!isEmpty(file)) {
		...
		formData.append("functionCall", "updateGeneralProgramPage");
		...
		$.ajax({
			...
			success: data => {
				...
				const response = JSON.parse(data);
				if (response.status == "success")
					window.location = `general_page.php?pageID=${pageID}&pageTitle=${programName}`;
				else 
					alert(response.message);
			}
		});
		...
		$.post("../../../utility/functions.php", {
			functionCall: "updateGeneralProgramPage",
			pageID: pageID,
